...
describes what users get. Pick a format with `-o`, or let worldbank choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
worldbank <command> -o table   # aligned columns for reading
worldbank <command> -o jsonl   # one JSON object per line, for piping
worldbank <command> -o json    # a single JSON array
worldbank <command> -o csv     # spreadsheet friendly
worldbank <command> -o tsv     # tab-separated
worldbank <command> -o url     # just the URL column
worldbank <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
worldbank <command> --fields id,title,url
```
...
```bash
worldbank <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
worldbank <command>            # a table, because this is a terminal
worldbank <command> | wc -l    # JSONL, because this is a pipe
```
